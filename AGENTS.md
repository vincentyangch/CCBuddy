...
CCBuddy is a personal AI assistant running on a Mac Mini, accessible via Discord and the dashboard. It supports both Codex and Codex backends, with the Codex migration now in progress.
...
- `packages/agent/src/backends/sdk-backend.ts` — Codex SDK integration
