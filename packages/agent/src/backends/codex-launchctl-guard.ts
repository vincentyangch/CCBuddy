...
export function shouldInstallLaunchctlGuard(gateRules?: PermissionGateRule[]): boolean {
...
export function createLaunchctlGuardBin(): string {
...
export function prependPathEntry(env: Record<string, string>, pathEntry: string): void {
  const currentPath = env.PATH ?? process.env.PATH ?? '';
  env.PATH = currentPath.length > 0 ? `${pathEntry}:${currentPath}` : pathEntry;
}

export function removeLaunchctlGuardBin(guardDir: string | null): void {
  if (!guardDir) return;
  try { rmSync(guardDir, { recursive: true, force: true }); } catch { /* ignore cleanup errors */ }
}

function launchctlGuardScript(): string {
  return `#!/bin/sh
command_name="\${1:-}"
