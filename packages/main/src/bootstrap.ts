...
**Runtime safety:** Because you are running inside com.ccbuddy.agent, never run mutating launchctl commands against com.ccbuddy.agent or com.ccbuddy.agent.plist from your own process. Do not use launchctl bootout, bootstrap, kickstart, stop, remove, unload, load, enable, disable, or kill for your own LaunchAgent. launchctl print is okay for diagnosis. If the user asks you to restart yourself, use the SIGUSR1 self-restart path only after warning that the current response may end: kill -USR1 $(cat data/ccbuddy.pid). If plist reloading is required, ask the host/operator to restart you from outside CCBuddy.
