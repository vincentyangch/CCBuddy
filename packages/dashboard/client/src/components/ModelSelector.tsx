...
export function ModelSelector() {
  ...
  const [serviceTierSource, setServiceTierSource] = useState<string>('');
  ...
  const reloadState = () => {
    return Promise.all([api.getModel(), api.getBackend()]).then(([modelData, backendData]) => {
      ...
      setServiceTier(modelData.service_tier ?? '');
      setServiceTierSource(modelData.service_tier_source ?? 'effective_only');
  ...
  const handleServiceTierChange = async (newValue: string) => {
    setSaving(true);
    setStatus('');
    try {
      await api.setModel({ service_tier: newValue || null });
      await reloadState();
      setStatus('Applied');
      setTimeout(() => setStatus(''), 2000);
    } catch (err) {
    ...
    <Panel className="p-4">
      ...
      {isCodex && (
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          ...
          <div>
            <div className="mb-2 flex items-center justify-between gap-2">
              <label htmlFor="runtime-service-tier-select" className="text-sm font-medium text-[color:var(--sd-text)]">Service tier</label>
              <StatusPill tone={serviceTierSource === 'runtime_override' ? 'warning' : 'neutral'}>
                {serviceTierSource === 'runtime_override' ? 'runtime override' : 'backend default'}
              </StatusPill>
            </div>
            <select
              id="runtime-service-tier-select"
              value={serviceTier}
